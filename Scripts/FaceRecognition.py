...
def get_resource(resource_path):
    ...
    # 1. Tenta caminhos relativos à raiz do projeto
    project_root = Path(__file__).parent.parent  # Sobe 2 níveis (Scripts -> ProjetoD)
    project_paths = [
        project_root / resource_path,
        project_root / "ProjetoD" / resource_path  # Caso o projeto tenha um diretório extra
    ]

    # 2. Tenta caminhos relativos ao executável (para versão compilada)
    if hasattr(sys, '_MEIPASS'):
        meipass_path = Path(sys._MEIPASS) / resource_path
        project_paths.insert(0, meipass_path)

    # 3. Tenta caminhos absolutos e do diretório de trabalho
        ...
        if counter != 0:
            ...
            if modeType != 3:
                ...
                if counter <= 10:
                    ...
                    phone_str = str(studentInfo['phone'])
                    last_four = phone_str[-4:] if len(phone_str) >= 4 else phone_str
                    ...
                    cv2.putText(imgBackground, last_four, (910, 625), cv2.FONT_HERSHEY_COMPLEX, 0.6, (100, 100, 100), 1)
